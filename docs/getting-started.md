...
| `--chunk-max-tokens` | Maximum approximate tokens per LLM chunk | `3000` |
...
| `--ocr-min-confidence` | Minimum OCR confidence from 0 to 1 | `0.3` |
...
The same tuning knobs are available from the CLI, so no Python wrapper is required:
