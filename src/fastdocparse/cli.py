...
def extract(
    ...
    chunk_max_tokens: int = typer.Option(3000, "--chunk-max-tokens", help="Maximum approximate tokens per LLM chunk (default: 3000)."),
    pdf_render_dpi: int = typer.Option(150, "--pdf-render-dpi", help="DPI used when rendering PDF pages for OCR (default: 150)."),
    max_image_dim: int = typer.Option(1536, "--max-image-dim", help="Maximum rendered image dimension in pixels (default: 1536)."),
    ocr_min_confidence: float = typer.Option(0.3, "--ocr-min-confidence", help="Minimum OCR confidence from 0 to 1 (default: 0.3)."),
    max_concurrent_chunks: int = typer.Option(1, "--max-concurrent-chunks", help="Maximum concurrent LLM chunk requests (default: 1)."),
):
